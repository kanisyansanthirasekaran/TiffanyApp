...
class UpscaledImageCreate extends Controller {
    ...
    public function create_ajax() {
        ...
        $uploaded_image_size_info = getimagesize(Uploads::get_full_url('upscaled_images') . $original_image);
        ...
        try {
            $response = \Unirest\Request::post(
                ...
                \Unirest\Request\Body::multipart([
                    ...
                    'image_file' => Uploads::get_full_path('upscaled_images') . $original_image
                ])
            );

            if($response->code >= 400) {
                /* Delete temp */
                unlink(UPLOADS_PATH . Uploads::get_path('upscaled_images') . $original_image);
        ...
        $upscaled_image_size_info = getimagesize(Uploads::get_full_url('upscaled_images') . $upscaled_image);
        $upscaled_image_size = $upscaled_image_size_info[0] . 'x' . $upscaled_image_size_info[1];

        /* Get size of the original image */
        $original_image_size_info = getimagesize(Uploads::get_full_url('upscaled_images') . $original_image);
