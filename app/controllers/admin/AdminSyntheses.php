...
class AdminSyntheses extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['user_id', 'project_id', 'language', 'voice_id', 'voice_engine', 'voice_gender'], ['name'], ['last_datetime', 'datetime', 'name', 'characters']));
        $filters->set_default_order_by('synthesis_id', settings()->main->default_order_type);
        ...
        /* Export handler */
        process_export_csv($syntheses, 'include', ['synthesis_id', 'project_id', 'user_id', 'name', 'input', 'file', 'language', 'voice_id', 'voice_engine', 'voice_gender',  'characters', 'api_response_time', 'datetime', 'last_datetime'], sprintf(l('syntheses.title')));
        process_export_json($syntheses, 'include', ['synthesis_id', 'project_id', 'user_id', 'name', 'input', 'file', 'language', 'voice_id', 'voice_engine', 'voice_gender', 'settings', 'characters', 'api_response_time', 'datetime', 'last_datetime'], sprintf(l('syntheses.title')));
            ...
            switch($_POST['type']) {
                case 'delete':
                    ...
                    foreach($_POST['selected'] as $synthesis_id) {
                        ...
                        $synthesis = db()->where('synthesis_id', $synthesis_id)->getOne('syntheses', ['user_id', 'synthesis']);

                        /* Delete file */
                        \Altum\Uploads::delete_uploaded_file($synthesis->synthesis, 'syntheses');
    ...
    public function delete() {
        ...
        if(!$synthesis = db()->where('synthesis_id', $synthesis_id)->getOne('syntheses', ['synthesis_id', 'user_id', 'name', 'synthesis'])) {
            redirect('admin/syntheses');
        }
